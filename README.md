...
Системные иконки (чекбокс, дроп-даун, крестик и подобное) подключаем в scss-файлах с помощью scss-функции: `get-icon($icon, $color`, например, `background-image: get-icon('vk', colors.$icon2);`

Вместо иконок-«изображений» используем иконку-заглушку - `default.svg`.

Если требуется сделать анимацию или как-то взаимодействовать с иконкой, допустимо вставить исходный код иконки в теге `<svg>` или использовать спрайты.
