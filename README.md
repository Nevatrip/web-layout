...
- Отступы — 2 пробела
- В JS, CSS — одинарные кавычки
- Для именования переменных, функций в JS, SCSS использовать camelCase
...
- Для CSS-классов, управляемых из JS, использовать префикс `js_dke_`. При этом создавать отдельный CSS-класс, а не модификатор
...
В проекте используется SASS-система управления иконками.

[Документация](https://css-tricks.com/creating-a-maintainable-icon-system-with-sass/)

Если иконка системная, которая не загружается менджерами: чекбокс, дроп-даун, крестик и подобное, то сначала иконку нужно привести к единому виду:

- Вписываем иконку самостоятельно или с дизайнером во viewBox 24x24. Для этого нужно ресайзить иконку в редакторе.
  Figma-документ с иконками [здесь](https://www.figma.com/file/rl854AwrdF6zlsA8lv8s38/Nevatrip-Icons-System?node-id=0%3A1&t=QxUiXcBVTLY0TGRy-1)
- В SVG-файле удаляем у тега `<svg>` все стилевые атрибуты, в итоге остаётся `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">`

Чтобы добавить иконку в проект, выполняем следующее:

- Копируем очищенный код иконки и кодируем его с помощью [онлайн-энкодера](https://yoksel.github.io/url-encoder/).
- Далее в файле `src/scss/_icons.scss` в SASS-map `$icons` добавляем ключ с названием иконки. В его значение помещаем код иконки из поля энкодера `Take encoded`, т. е. без префикса `data:image`.
- Значение свойства в коде иконки, отвечающее за цвет, нужно заменить на плейсхолдер `%%COLOR%%`. Например: `fill="%%COLOR%%"`.

Чтобы использовать иконку, нужно подключить её в свойстве `background`. Для этого:

- Импортируем файл с иконками в ваш стилевой файл с помощью `@use '../icons' as *;`
- Используем иконку с помощью функции `get-icon` — `background: get-icon('icon-name', colors.$link) no-repeat`. Вместо `icon-name` указываем название иконки из SASS-map, вторым аргументом указываем цвет иконки (если не указать, по умолчанию будет белый цвет).

Иконки-«изображения», загружаемые менджерами: логотипы, какие-то схематичные картинки и подобное, заменяются в вёрстке заглушкой с помощью тега `<img src="src/icons/default.svg">`. Файл `default.svg` — заглушка для контентных иконок.

Если требуется сделать сложную анимацию, допустимо вставить исходный код иконки в теге `<svg>`.
...
При появлении новых иконок в проекте, добавляем их в файл `src/pages/trip-icons/trip-icons.hbs`, в один из трех разделов: в "Системные иконки", в "Спрайт" или в "Прочие".
